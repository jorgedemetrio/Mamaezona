...
class WbampModel_Manager
{
	...
	public function parseAndUpdateRequest()
	{
		...
		// if standalone mode (or forced mobile), this is an AMP request
		if (
		...
		// else we need to look for the 'amp' suffix. But we also do that if
		// this is a mobile request and admin choose to force AMP for mobile users.
		// This is because in the latter situation, an AMP page must be reached
		// both through /my-page and /my-page/amp
		if (
			!WbampHelper_Runtime::isStandaloneMode()
			||
			WbampHelper_Runtime::isForcedMobile()
		...
		if ($this->_isAmpRequest)
		{
			...
			$this->_canonicalUrl = WbampHelper_Runtime::$base . $truncatedPath . ($hasTrailingSlash && !empty($truncatedPath) ? '/' : '') . $this->_query;
		}
	...
	/**
	 * Whether the current request had the "amp" suffix in the URL.
	 *
	 * @return bool
	 */
	public function hasSuffix()
	{
		return !empty($this->_currentSuffix);
	}
