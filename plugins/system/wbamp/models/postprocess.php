...
class WbampModel_Postprocess
{
	 ...
	 *
	 * Lastly, handle form shortocodes:
	 *
	 * {wbamp-form start}....an HTML form...{wbamp-form end}
	 * or
	 * {wbamp-form start template="mustache"}....an HTML form...{wbamp-form end}
	 *
	 * NB: the action form attribute is not modified, it must be valid (ie action for GET, or action-xhr for POST)
	 *
	 * @param $rawContent
	 *
	 * @return mixed
	...
	private function _cleanContent($rawContent, $dom)
	{
		...
		$regExp = '#{wbamp-form start([^}]*)}(.*){wbamp-form end}#iuUs';
		$content = preg_replace_callback($regExp, array($this, '_processFormTags'), $content, -1, $replaceCount);
		$modified = $modified || !empty($replaceCount);
	...
	/**
	 * perg replace callback, protect a form, and include required scripts.
	 *
	 * @param array $match
	 *
	 * @return string
	 */
	private function _processFormTags($match)
	{
		if (!empty($match[1]))
		{
			$attributes = JUtility::parseAttributes($match[1]);
			$template = empty($attributes['template']) ? '' : trim($attributes['template']);
			if (!empty($template))
			{
				// ability to handle mustache templates
				WbampHelper_Content::addTemplates(
					array(
						'amp-' . $template => sprintf(WbampModel_Renderer::AMP_SCRIPTS_PATTERN, $template, WbampModel_Renderer::AMP_SCRIPTS_VERSION),
					)
				);
			}
		}

		// make sure we have a "form" script loaded
		WbampHelper_Content::addScripts(
			array(
				'amp-form' => sprintf(WbampModel_Renderer::AMP_SCRIPTS_PATTERN, 'form', WbampModel_Renderer::AMP_SCRIPTS_VERSION)
			)
		);

		// finally replace the form content
		return WbampHelper_Content::protectForm(
			$match[2]
		);
	}
	...
	private function _convertToAmp($dom)
	{
		...
		{
			$sanitizer = new $sanitizer_class($dom, array_merge($args, $this->_args));
